import { telegram } from "../../data/data";
import "atropos/css/min";
import Atropos from "atropos/react";

function TelegramBot() {
  const handleClick = (telegramUrl) => {
      ...
      <div className="w-full bg-slate-800 px-4 sm:px-5 md:px-6 lg:px-6 flex flex-col justify-center items-center rounded-xl">
        <div className="w-full flex flex-col justify-center items-center h-auto mb-8 sm:mb-12 md:mb-14 lg:mb-16">
          ...
          <p className="w-full text-slate-300 py-3 sm:py-4 md:py-5 lg:py-6 text-sm sm:text-base md:text-lg lg:text-xl text-justify">
            ...
            نیازسنجی‌های پروژه انتخاب می‌شوند و در اسرع وقت در اختیار شما قرار میگیرند مثل ربات های تلگرام
          </p>
        </div>
            ...
            {telegram.map((template) => {
              return (
                <Atropos
                  ...
                  highlight={false}
                  key={template.id}
                  onClick={() => handleClick(template.telegramUrl)} // Pass template URL to handleClick function
                >
                  <div className="w-full  flex justify-center items-center border border-sky-300  rounded-lg">
